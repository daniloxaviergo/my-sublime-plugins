...
class IncreaseFoldCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    ...
    if fold_level and fold_level < 7:
      self.increase_and_save(fold_level, jjson)
    else:
      self.increase_and_save(0, jjson)

  def increase_and_save(self, fold_level, jjson):
